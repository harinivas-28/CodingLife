...
// Remove Employee
const remInd = employees.findIndex(emp => emp.name==="Emily");
if(remInd!==-1){
    employees.splice(remInd, 1);
}
// Low Performers
const low = employees.filter(emp => {
   return emp.ratings.some(rat => rat<60); 
});
// console.log(low);
const lowPerformers = low.map(emp => emp.name);
console.log("Low Performers: "+lowPerformers);
// Computing Average
const avg = employees.map(emp => {
   const avg = emp.ratings.reduce((sum, rat)=>sum+rat, 0) / emp.ratings.length;
   return {...emp, average: avg};
});
// console.log(avg);
// Top performers
const top = avg.filter(emp => emp.average>85);
top.sort((a, b)=> b.average-a.average)
// console.log(avg);
// Format (mapping)
const form = top.map(emp=>`${emp.name}: ${emp.average.toFixed(2)}`);
console.log("Top Performers: "+form);
