...
class Customer {
    ...
    private void setMissedPayments(int missedPayments){
...
class CreditRiskAnalyzerImpl implements RiskAnalyzer {
    ...
    public CreditReport analyzeRisk(Customer customer) {
        ...
        double missedPaymentsScore = 1-Math.min(1, customer.getMissedPayments()/10);
        double score = (0.4 * incomeScore) + (0.4 * debtScore) + (0.2 * missedPaymentsScore) * 100;
        String riskLevel;
        if(score>=80){
            riskLevel = "Low";
        } else if(score<50){
        ...
        return new CreditReport(customer, score, riskLevel);
    }
}


public class Day80P1_Design {
