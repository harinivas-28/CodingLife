...
public class Day16P1 {
    ...
    public static String F(String s){
        int i = 0;
        while(i<s.length() && set.contains(Character.toUpperCase(s.charAt(i))) && set.contains(Character.toLowerCase(s.charAt(i)))){
            i++;
        }
        String l = (i==0) ? F(s.substring(0, i)) : "";
        String r = (i<s.length()) ? F(s.substring(i, s.length())) : "";
