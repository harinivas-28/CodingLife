...
public class Day36P2_Bt {
    public static void main(String[] args){
        ...
        // 3 0 2 1 2
        sc.close();
        System.out.println(dfs(arr, pos, new boolean[n]));
    }
    private static boolean dfs(int[] arr, int pos, boolean[] vis){
        if(pos<0 || pos>=arr.length || vis[pos]) return false;
        if(arr[pos]==0) return true;
        vis[pos] = true;
        return dfs(arr, pos+arr[pos], vis) || dfs(arr, pos-arr[pos], vis);
    }
}