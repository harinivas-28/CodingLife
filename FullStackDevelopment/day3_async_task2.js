...
function fetchAvailableMovies() {
  ...
  async function processBooking(userId) {
    ...
    const [movies, checkUser] = await Promise.all([
    ...
    const showTime = await selectShowtime(movies[0]);
    console.log(showTime);
    
    const makePay = await makePayment();
    console.log(makePay);
  
    // Concurrent Activities: Send email and SMS confirmations
    const [confMail, confSMS] = await Promise.all([
        sendEmailConfirmation(),
        sendSMSConfirmation(),
    ]);
    console.log(confMail);
    console.log(confSMS);
